...
<script>
    $(function () {
        ...
        $.get("/organization/tree", {type: "resource"}, function (data) {
            $('#organization-tree-resource').treeview({
                //开通超链接效果
                enableLinks: false,
                backColor: '#fff',
                showBorder: false,
        ...
        search.bind('blur',function(){
            ...
            $('#resourceInput').removeClass("width-change");
        });

        //点击搜索框里面的搜索图标触发查询
        var searchBtn = $('#search-data-icon');
        ...
        //显示搜索框
        var searchShowBtn = $('#search-show-icon');
        searchShowBtn.click(function(){
            $('#tab-link').css("display", "none");
            $('#tab-search').css("display", "block");
            $('#resourceInput').focus();
        });
...
//页面加载完成初始化结束
    ...
    function searchTree(value){
        ...
        //树菜单搜索
        $('#organization-tree-resource').treeview('search',[pattern,
            {
                ignoreCase: true,
                exactMatch: false,
                revealResults: true,
            ...
            <div style="padding:10px">
                <div style="width: 570px; height: 440px;">
                    ...
                    <div class="select-menu">
                        <div class="search-input" style="width: 100%; display: block" id="tab-link">
                            <a class="active" href="#organizationPanel" data-toggle="tab">部门</a>
                            <a href="#resourcePanel" data-toggle="tab">人员</a>
                            <i class="fa fa-search" id="search-show-icon"></i>
                        </div>
                        <div class="search-input" style="width: 100%; display: none" id="tab-search">
                            <input id="resourceInput" placeholder="输入员工姓名或部门名称，多个条件用逗号,分割">
                            <i class="fa fa-search" id="search-data-icon"></i>
                        </div>
                    </div>
                    <div id="resource-tab" class="tab-content" style="border: solid 1px #e0e0e0;border-bottom:none;border-top: none;">

                        <%--选择部门列表--%>
                        <div class="tab-pane fade in active" id="organizationPanel" style="position: relative;height: 320px;">
                            <div class="resource-menu">
                                <div id="organization-tree" class="organization-tree" style="padding: 5px 0">
                                </div>
                            </div>
                        </div>
                        <%--选择人员列表--%>
                        <div class="tab-pane fade in " id="resourcePanel" style="position: relative;height: 320px;">
                            <div class="resource-menu">
                                <div id="organization-tree-resource" class="organization-tree" style="padding: 5px 0">
                                </div>
                            </div>
