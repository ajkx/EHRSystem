...
<script>
    $(document).ready(function() {
        ...
        $(".update-btn").click(function(){
            $.get("/${url}/update/"+$(this).attr("data-topic-id"),function(data,status){
                $("#createModal").modal("show");
                $('#topicid').val(data.id);
                $(".modal-title").text("${topic}信息更改");
                $(".form-horizontal").attr("action","/${url}/update");
                <c:forEach items="${editlist}" var="editlist">
                $("#${editlist}").val(data.${editlist});
                </c:forEach>
            });
        });
        $('#createModal').on('hide.bs.modal', function () {
            $(".modal-title").text("${topic}信息创建");
            if($(".form-horizontal").attr("action") == "/${url}/update"){
                <c:forEach items="${editlist}" var="editlist">
                $("#${editlist}").val("");
                </c:forEach>
            }
            $(".form-horizontal").attr("action","/${url}/create");
        });

        $('.delete-btn').click(function(){
            changeId = $(this).attr("data-topic-id");
            $('#confirmModal').modal("show");
        });

        $('.btn-confirm').click(function(){
            $.get("/${url}/delete/"+changeId,function(data){
